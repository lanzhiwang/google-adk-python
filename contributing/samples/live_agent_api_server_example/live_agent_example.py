...
try:
    ...
    print(
        "WARNING: Sounddevice or numpy not found. Audio RECORDING will be" " disabled."
    )
...
def create_text_request_payload(text: str) -> str:
    ...
    logging.debug(f"Created LiveRequest text payload: {json.dumps(live_request_data)}")
...
class AudioStreamingComponent:
    ...
    async def start_audio_streaming(
        ...
        if not AUDIO_RECORDING_ENABLED:
            ...
            is_streaming_audio = False  # Ensure flag is correctly set if we bail early
            return
            ...
            while is_streaming_audio:
                try:
                    ...
                    if audio_data_bytes:
                        ...
                        logging.warning(
                            "Empty audio data chunk from queue, not sending."
                        )
...
class AgentResponseAudioPlayer:
    ...
    def _play_audio_pyaudio_handler(self, audio_bytes: bytes, mime_type_full: str):
...
async def websocket_client():
    ...
    if not session_ok:
        logging.error(f"Critical: Could not ensure session '{SESSION_ID}'. Aborting.")
        return
    ...
    uri = f"ws://{SERVER_HOST}:{SERVER_PORT}/run_live?{urllib.parse.urlencode(params, doseq=True)}"
        ...
        async with websockets.connect(
                                    ...
                                    for part in content_obj["parts"]:
                                        if (
                                            ...
                                            if (
                                                ...
                                                and isinstance(
                                                    inline_data["mimeType"], str
                                                )
                                                and inline_data["mimeType"].startswith(
                                                    "audio/"
                                                )
                ...
                while True:
                    ...
                    try:
                        ...
                        if user_input.lower() == "quit":
                            ...
                            if audio_stream_task and not audio_stream_task.done():
                                logging.warning(
                                    "Sender: Audio stream is already running."
                                )
                                continue
                            ...
                            if audio_stream_task and not audio_stream_task.done():
                                ...
                                audio_stream_task = None
                                logging.info(
                                    "Sender: Audio streaming task stopped and joined."
                                )
                            else:
                                logging.warning(
                        ...
                        # If you need it, uncomment the block from my previous response.
                            ...
                            try:
                                ...
                                print(
                                    f"Successfully sent {os.path.basename(filepath)}."
                                )
                        ...
                        if audio_stream_task and not audio_stream_task.done():
                            ...
                            try:
                                ...
                                logging.error(
                                    f"Error during stream stop on conn close: {ex}"
                                )
                            audio_stream_task = None
                        break
            ...
            for task in all_tasks_to_await:
                try:
                    ...
                    logging.error(
                        f"Error awaiting main task {task.get_name()}: {e}",
                        exc_info=True,
                    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.info("Script's main execution block finished. Shutting down logging.")
        logging.shutdown()
