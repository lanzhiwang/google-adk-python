...
async def analyze_cache_performance_from_sessions(
    ...
    try:
        ...
        if cached_analysis["status"] == "active":
            ...
            print(f"     Requests with cache: {cached_analysis['requests_with_cache']}")
            print(
...
def calculate_averaged_results(
    ...
    averaged_result = {
        ...
        "model": model_name,
        "individual_runs": (all_results),  # Keep all individual results for reference
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="ADK Cache Performance Experiment")
    parser.add_argument(
        "model",
    ...
    if args.repeat == 1:
        ...
        print(f"    Cache Hit Ratio: {uncached_exp['cache_hit_ratio_percent']:.1f}%")
        print(
            "    Cache Utilization:"
