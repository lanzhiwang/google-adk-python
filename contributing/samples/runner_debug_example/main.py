...
async def example_separate_sessions():
    ...
    print("\n-- Bob's session --")
    await runner.run_debug("Calculate 100 / 5", user_id="bob", session_id="bob_debug")
    ...
    for i, event in enumerate(events):
        if event.content and event.content.parts:
            for part in event.content.parts:
                if part.text:
                    print(
                        f"  Event {i+1}: {event.author} - Text: {len(part.text)} chars"
                    )
...
async def example_with_run_config():
    ...
    await runner.run_debug("Explain what tools you have available", run_config=config)
