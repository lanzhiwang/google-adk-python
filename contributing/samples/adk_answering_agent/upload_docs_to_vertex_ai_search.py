...
def main():
    ...
    if not VERTEXAI_DATASTORE_ID:
        print(
            "[ERROR] VERTEXAI_DATASTORE_ID environment variable not set." " Exiting...",
            file=sys.stderr,
        )
        return 1
    ...
    for gcs_prefix in GCS_PREFIX_TO_ROOT_PATH:
        ...
        if not cleanup_gcs_prefix(GOOGLE_CLOUD_PROJECT, GCS_BUCKET_NAME, gcs_prefix):
    ...
    if not import_from_gcs_to_vertex_ai(VERTEXAI_DATASTORE_ID, GCS_BUCKET_NAME):
        print(
            "[ERROR] Failed to import docs from GCS to Vertex AI Search." " Exiting...",
            file=sys.stderr,
        )
        return 1
