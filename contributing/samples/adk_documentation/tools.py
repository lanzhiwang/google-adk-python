...
def list_releases(repo_owner: str, repo_name: str) -> Dict[str, Any]:
    ...
    url = f"{GITHUB_BASE_URL}/repos/{repo_owner}/{repo_name}/releases?per_page=100"
...
def list_directory_contents(directory_path: str) -> Dict[str, Any]:
    ...
    """
    print(f"Attempting to recursively list contents of directory: {directory_path}")
...
def get_issue(repo_owner: str, repo_name: str, issue_number: int) -> Dict[str, Any]:
    ...
    """
    url = f"{GITHUB_BASE_URL}/repos/{repo_owner}/{repo_name}/issues/{issue_number}"
    try:
